...
6. **Keep commit messages short.** A subject line under ~70 characters, then at most three or four lines of body — only what someone scanning `git log` needs. No bullet inventories, no measurements, no reasoning about what was wrong before. That material belongs in the code comment next to the thing, in this file, or in the skills, where it stays discoverable; in a commit message it is write-only. If the change genuinely needs more explanation than that, the explanation should be committed *as a file*.
7. **Disagreeing with a subagent is fine — deadlocks go to the user.** Subagent output is advice, not instruction, and is sometimes wrong. Push back when it's wrong. But if a disagreement persists after one round of back-and-forth, stop and present both positions for the user to decide rather than picking a side.
...
`anuj_preferences.md` records the site owner's design preferences — figures, animation, layout, video, and how he wants to be worked with — reconstructed from working sessions and marked for whether each line was stated or inferred. Read it before designing anything new. It is **local-only and gitignored**, like `TODO.md`: present in the working copy, absent from a fresh clone. Do not link to it from anything that ships.
...
| `assets/` | yes | what the site actually fetches: four `*.mp4` web derivatives, their four posters, three OG cards, six `photo-*.jpg` hero slides |
| `cv/` | partly | the CV work. `design-4-scan` (the CV the site serves) and `design-10-newspaper` (the broadsheet the nav links) ship **three files each** — `cv.html`, `cv.pdf`, `NOTES.md` — allowlisted in `.gitignore`; every other design, and every page raster and probe, is ignored |
| `favicon.ico` | yes | the mark as a FILE, for Google Search only — it cannot use the `data:` URI the pages declare. Browsers never request it. Regenerate with `tools/mkfavico.py`; keep it in step with `tools/mkfav.js` |
| `AnujKankani-CV.pdf` | yes | the published CV. A **copy** of `cv/design-4-scan/cv.pdf`; a suite check byte-compares them, because re-rendering without re-copying leaves the site serving a stale PDF with no visible symptom |
| `type-tests/` | **no** | typeface comparison pages and the iframe measurement harnesses. Regenerate with `tools/mktypetests.py` (also ignored) |
...
- [index.html](index.html) — the whole site. Nav, hero, and seven live sections in document order: `about`, `publications`, `software`, `awards`, `bob`, `explainers`, `widgets`, plus `contact` on the `<footer>` rather than a `<section>`. Grepping `<section id=` returns **eight** matches — the seventh live one plus `random`, which is commented out — so the count matches neither the nav nor the live sections. Strip comments before trusting a grep here.

  Section `id`s are the anchor targets, so **renaming an `id` means editing `.nav-links` in the same change**. That has already gone wrong once: `<section id="research">` was renamed to `id="awards"` and its prose folded into About, but the nav kept `href="#research"` — a dead link that clicked to nothing and went unnoticed for days, while the new Awards section had no nav entry at all. The entry now reads Awards and points at `#awards`. Verify anchors programmatically (every `href="#x"` resolves to an `id="x"` in the comment-stripped HTML), not by clicking.

  The nav order follows document order, so moving a section means editing both. (Random is written but commented out — see Content editing.)
...
All three pages carry `description` / Open Graph / `twitter:card` / `canonical` tags and an inline SVG favicon. Three things fail **silently** if changed: `og:image` must be an **absolute** URL (scrapers do not resolve relative paths and report nothing); the *declared* favicon must stay a `data:` URI (a **linked** icon file would be the first runtime fetch other than the font); and `favicon.ico` must nonetheless exist at the root, because **Google Search cannot use a `data:` URI favicon** and falls back to that path — without it, results show a generic globe with nothing on the page to explain why. Browsers that already have the inline icon never request the file, so it costs no fetch; measured against a server log. All three are covered by the `every page :: social preview and favicon` suite.
...
The chirp was *illustrative only* — hand-tuned, not BOB output. `og-home.jpg` was regenerated from the hero figure on 2026-08-17 (the card now inlines the *same* SVG the page does, lifted out of `index.html` rather than redrawn, so it cannot drift again). **The favicon is now the only place the chirp survives** — `tools/mkfav.js` still draws it, so the tab mark and the page disagree. Worth deciding whether the chirp stays the site's mark or the favicon follows the card.

### Responsive and touch

Five rules here were each a shipped bug; changing any of them back reintroduces it.

**The nav's scrollable breakpoint is `max-width:963px`, and it is a measured number, not a round one.** The desktop nav needs 964px: brand 98 + links 674 + **three** 34px controls + their 8px cluster gaps + a 20px gap + the wrap's 56px padding. The number is a function of what is in the bar and moves in **both** directions: 924px with one toggle, 966px when the type toggle arrived, 1008px with the newspaper link, then back to 964px when the dead `Research` nav entry was relabelled `Awards` — two characters off one link is 16px off the strip. Re-measure after any nav edit, a pure rename included. At the `.nav-right` gap of 20px the third control needed 1020px — four pixels past iPad landscape — which is why `.nav-tools` tightens the gap *between* the icons to 8px while leaving 20px to the links. It used to be 720px, which left the whole **721–919px** band broken — the links wrapped onto two lines throughout it, and below ~850px the nav also pushed the page into horizontal scroll with the theme toggle off-screen. 768px (iPad portrait) sat squarely inside that. Below the breakpoint the links get `white-space:nowrap` and the strip scrolls horizontally; both halves are required.
